//! `~/.ambry/history.json` — query history, newest first, capped at 500.
//!
//! A corrupt file makes load (and append) fail — the TS reader has no
//! try/catch, bug-compatible.
...
/// Prepend (the TS `unshift`), truncate to 500, write.
...
mod tests {
    ...
    fn entry(sql: &str) -> HistoryEntry {
        HistoryEntry {
            ...
            executed_at: crate::types::iso_now(),
            execution_time: 1,
            rows_affected: 0,
            error: None,
        }
