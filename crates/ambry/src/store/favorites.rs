//! `~/.ambry/favorites.json` — saved queries.
//!
//! Like history, a corrupt file makes every operation fail (bug-compatible).

use crate::store::paths;
use crate::types::{iso_now, new_uuid, Favorite};
...
/// `favorites:save` — fills id/createdAt when absent (TS `||`, so empty
/// strings count as absent), upserts by id.
pub fn save(
    id: Option<String>,
    name: &str,
    sql: &str,
...
/// `favorites:delete` — Ok(false) when the id wasn't stored.
