...
/// `connection:save` — fills a uuid when the id is empty, replaces in place
/// by id or appends, returns the saved connection.
...
/// `connection:delete`'s file step — false when the id wasn't stored.
...
/// Test-only: run `f` with `AMBRY_DIR` pointed at a fresh temp dir. The env
...
pub(crate) fn testenv<T>(f: impl FnOnce() -> T) -> T {
    ...
    let dir = std::env::temp_dir().join(format!("ambrytest{}", new_uuid()));
    std::fs::create_dir_all(&dir).unwrap();
    std::env::set_var("AMBRY_DIR", &dir);
    let result = f();
    std::env::remove_var("AMBRY_DIR");
