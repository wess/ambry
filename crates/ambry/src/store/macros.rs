//! `~/.ambry/macros.json` — stored macros. The host only stores them;
//! execution lives in the UI.

use serde_json::Value;

use crate::store::paths;
use crate::types::{iso_now, new_uuid, Macro, MacroStep};
...
/// `macros:save` — fills id/createdAt when absent (TS `||`), upserts by id.
pub fn save(
    id: Option<String>,
    name: &str,
    steps: Vec<MacroStep>,
...
/// `macros:delete` — filter-out; the TS handler always returns true.
...
/// `macros:export` — the macro pretty-printed.
...
/// `macros:import` — parse when given a JSON string; the imported macro
/// always gets a fresh id and createdAt, then is appended.
