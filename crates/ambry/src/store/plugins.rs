//! `~/.ambry/plugins/` + `~/.ambry/plugins.json` — inert plugin files.
...
const REGISTRY_URL: &str =
    "https://raw.githubusercontent.com/ambry-app/plugins/main/registry.json";

fn dir() -> PathBuf {
    let d = paths::ambry_dir().join("plugins");
...
/// `plugins:list` — every directory with a parseable manifest.json;
/// enabled unless the config says exactly `false` (TS `config[name] !== false`).
...
/// `plugins:toggle` — set `config[name] = enabled`.
...
/// `plugins:registry` — HTTP error (curl exit 22) or unparseable body → [];
/// network/spawn failure → the hardcoded fallback list.
...
fn fallback() -> Vec<PluginManifest> {
    [
        ("ambry-mongodb", "MongoDB driver", "driver"),
        ("ambry-redis", "Redis driver", "driver"),
        ("ambry-duckdb", "DuckDB driver", "driver"),
        ("ambry-xlsx-export", "Excel XLSX export", "export"),
        ("ambry-dracula", "Dracula theme", "theme"),
        ("ambry-nord", "Nord theme", "theme"),
    ]
    .into_iter()
...
/// `plugins:install` — mkdir; when a `url` rides along in the manifest's
/// extra fields, download it to `index.ts`; write a default manifest.json
...
pub fn install(data: &PluginManifest) -> Result<(), String> {
    ...
    if !manifest_path.exists() {
        // TS `||` defaults: empty strings fall back too.
...
/// `plugins:uninstall` — rm -rf, errors swallowed.
...
mod tests {
    ...
    fn install_writes_default_manifest_once() {
        testenv(|| {
            ...
            // A pre-existing manifest is left alone.
    ...
    fn fallback_list_is_verbatim() {
        ...
        assert_eq!(list[0].name, "ambry-mongodb");
        assert_eq!(list[3].kind, "export");
        assert_eq!(list[5].name, "ambry-nord");
        assert!(list.iter().all(|m| m.version == "0.1.0" && m.author.as_deref() == Some("ambry")));
    }
}
