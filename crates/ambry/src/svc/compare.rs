//! Schema diffing — `src/host/tables/schemacompare.ts`. The generated SQL
//! migrates target toward source.

use crate::types::{ColumnInfo, SchemaDiff};

/// JS `new Map(entries)` semantics: duplicate keys keep the first position
/// but take the last value.
...
fn get<'a, T: Copy>(map: &[(&'a str, T)], key: &str) -> Option<T> {
    ...
    for (name, source_table) in &source_map {
        ...
        for (col_name, source_col) in &source_cols {
            ...
            if !changes.is_empty() {
                ...
                // nullability/default changed — bug-compatible.
                let mut sql = format!(
