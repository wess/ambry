//! CSV parsing and INSERT generation — `src/host/tables/csvimport.ts`.
...
/// dropped. Quoted fields cannot span lines (bug-compatible).
...
pub fn csv_to_insert_sql(table: &str, csv: &str, delimiter: &str) -> Vec<String> {
...
mod tests {
    ...
    fn produces_one_insert_per_row_with_quoted_identifiers() {
        let sql = csv_to_insert_sql("users", "id,name\n1,Alice", ",");
        assert_eq!(
            sql,
    ...
    fn treats_numeric_looking_values_as_numbers_and_others_as_strings() {
        let sql = csv_to_insert_sql("t", "a,b,c\n42,3.14,hello", ",");
        assert_eq!(
            sql[0],
