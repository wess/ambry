//! Mock row generation — `src/host/tables/mockdata.ts`. Word lists and value
//! rules are verbatim; rows that end up empty are still emitted (filtering
//! happens at insert).
...
use crate::types::{new_uuid, ColumnInfo};
...
/// Random float rounded to 2 decimals — `+(x).toFixed(2)`.
...
/// None means the key is omitted from the row (TS `undefined`).
fn generate_value(col: &ColumnInfo, row_index: usize, rng: &mut impl Rng) -> Option<Value> {
    ...
    if t.contains("text") || t.contains("varchar") || t.contains("char") || t.contains("string") {
        ...
        return Some(Value::from(
            sentence.chars().take(len).collect::<String>(),
        ));
    }
...
mod tests {
    ...
    fn produces_email_shaped_values_for_email_columns() {
        ...
        assert!(rows
            .iter()
            .all(|r| r["email"].as_str().unwrap().contains('@')));
    }
