//! The data grid (`src/app/components/grid`). A custom grid — not guise's
//! static `Table` — so cells can be selected, sorted, and edited inline.
//! Editing a cell queues a `PendingChange::Update`; the commit flow lives in
//! the Data panel.
