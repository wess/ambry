...
//! workspace is open and is provided by the workspace page. Panels read the
//! signals they care about and `watch` them — the same lifting-state-up
//! shape as the original React routes.
...
/// Filter panel state: a draft being edited and the applied set actually
/// sent with `table:rows`.
...
pub enum PendingChange {
    Update {
        table: String,
        primary_key: crate::types::Row,
        changes: crate::types::Row,
    },
    Insert {
        table: String,
        row: crate::types::Row,
...
pub struct WorkspaceState {
    ...
    // Data-tab state, reset when a table is selected (shell spec §6.5).
    pub page: Signal<u64>,
    ...
    // Workspace-level modals (opened from the header / tab strip).
...
impl WorkspaceState {
    ...
    /// FK navigation: same resets, but pre-seeded with one `=` filter and
    /// the panel open (shell spec §6.5).
    pub fn navigate_fk(&self, cx: &mut gpui::App, table: &str, column: &str, value: &str) {
        let filter = FilterState {
            conditions: vec![FilterCondition {
                id: crate::types::new_uuid(),
    ...
    pub fn record_step(&self, cx: &mut gpui::App, action: &str, params: serde_json::Map<String, serde_json::Value>) {
