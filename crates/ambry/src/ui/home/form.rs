//! The new/edit connection form (`src/app/components/connections/form.tsx`).
//!
//! A modal over the host's General settings. SSL and SSH are round-tripped
//! from the edited connection untouched (their editors land in a later pass).
...
impl ConnectionForm {
    pub fn new(initial: Option<StoredConnection>, cx: &mut Context<Self>) -> Self {
        ...
        let g = |get: fn(&StoredConnection) -> String| {
            initial.as_ref().map(get).unwrap_or_default()
        };
    ...
    fn test(&self, cx: &mut Context<Self>) {
        ...
        task::run(
            cx,
            move || host.test_connection(&conn),
        ...
        let actions = Group::new()
            ...
            .child(
                Group::new()
                    ...
                    .child(
                        Button::new("form-save", if editing { "Save" } else { "Create" })
