//! One connection card (`src/app/components/connections/card.tsx`). Rendered
//! by `Home` so its action buttons can dispatch back through `Home`'s listeners.
    ...
    pub(super) fn card(
        ...
        let edit = ActionIcon::new(SharedString::from(format!("edit-{}", conn.id)), "✎")
            ...
            .on_click(cx.listener(move |this, _, _, cx| {
                this.open_form(Some(conn_edit.clone()), cx)
            }));
        ...
        let delete = ActionIcon::new(SharedString::from(format!("delete-{}", conn.id)), "🗑")
            ...
            .size(Size::Sm)
            .on_click(cx.listener(move |this, _, _, cx| {
                this.request_delete(conn_delete.clone(), cx)
            }));
