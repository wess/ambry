//! The home page — the connection list (`src/app/routes/index.tsx` plus the
//! `connections/list` component). Loads the stored connections through the
//! host, groups them, and drives new / edit / delete / connect.
...
use crate::ui::home::form::{ConnectionForm, ConnectionFormEvent};
...
impl Home {
    ...
    /// Refetch the stored connections off-thread (`connection:list`).
    ...
    /// Connect (`connection:connect`), then navigate to the workspace. A
    /// failure clears the spinner and toasts the error, staying on home.
    ...
    fn save(&self, conn: StoredConnection, cx: &mut gpui::App) {
        ...
        task::run(
            cx,
            move || host.save_connection(&conn),
    ...
    fn confirm_delete(&mut self, cx: &mut Context<Self>) {
        if let Some(conn) = self.pending_delete.take() {
            ...
            task::run(
                cx,
                move || host.delete_connection(&id),
...
/// Group by `group` (falling back to "Connections"), preserving first-seen
/// order — the `useMemo` grouping in the list component.
...
impl Render for Home {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let header = Stack::new()
            ...
            .child(Text::new("Ambry").size(Size::Lg).bold())
        ...
        if conns.is_empty() {
            ...
            page = page.child(
                Center::new()
                    .child(div().w_full().max_w(px(900.0)).child(list)),
            );
        }
